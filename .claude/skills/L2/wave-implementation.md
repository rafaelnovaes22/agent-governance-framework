...
description: Implementa em ondas verticais finas, verificando entre cada onda com foundry-doctor. Use quando executar /novais-digital:implement, ao implementar qualquer mudança que toque mais de um arquivo, ou sempre que sentir a tentação de escrever muito código de uma vez antes de testar. Adaptado de incremental-implementation (agent-skills).
tier: 2
vocabulary_aliases: [L2, wave, incremental, ondas]
linked_principles: [C3, C6, C8]
version: 1.0.0
activation:
  keywords: [implement, onda, wave, incremental, tarefa, task, /novais-digital:implement]
  explicit_invocation: "@wave-implementation"
---

# Wave Implementation (Foundry)

## Visão Geral

Construa em ondas verticais finas — implemente uma onda, verifique com `foundry-doctor`, commit, avance. Não implemente um feature inteiro em um passe. Cada onda deve deixar o sistema num estado funcional e auditável.

No contexto Foundry, "onda" tem dois significados sobrepostos:

1. **Ondas do pipeline `/novais-digital:tasks`** — Onda 1 (scaffolding) → Onda 2 (prompt/service) → Onda 3 (eval/test) → Onda 4 (SHADOW/PILOT prep) → Onda 5 (métricas) → Onda 6 (CI/CD)
...
- Executando tarefas de `/novais-digital:tasks` — especialmente Ondas 1–3
...
```
Implementar ──→ foundry-doctor ──→ Commit ──┐
     ▲                                     │
     └───────────── Próxima fatia ◄────────┘
```
...
2. **Verificar** com `bash scripts/foundry-doctor.sh` — zero FAILs antes de avançar
3. **Commit** com mensagem no formato: `type(scope): desc`
4. **Avançar** para a próxima fatia

`foundry-doctor` é o gate entre commits. Não pule, não faça "depois eu arrejo".

## Estratégias de Fatiamento para Foundry
...
Fatia 1: Schema + adapter C7 + observe() mínimo
    → foundry-doctor passa, trace aparece no LANGSMITH

Fatia 2: Lógica de negócio + casos de erro
    → foundry-doctor passa, casos de erro testados

Fatia 3: Eval seed (3+3 casos mínimos para C4)
    → foundry-doctor passa, eval suite verde

Fatia 4: Prompt compression e cost check (C3)
    → foundry-doctor passa, cost ≤ 25% do preço
```

Cada fatia entrega funcionalidade testável de ponta a ponta.

### TDD-Red First (Foundry-10)
...
Fatia 1: Implementar o mínimo para passar (green phase)
    → foundry-doctor + testes passam

Fatia 2: Refatorar sem quebrar (refactor)
    → foundry-doctor ainda passa
```

Nunca pule o red phase — é um gate do Foundry-10 verificado em CI.
...
Toque apenas o que a tarefa requer. No Foundry, há uma armadilha extra: **não adicione código específico por tenant enquanto implementa um feature genérico**.
...
### Regra 2: foundry-doctor Verde Entre Commits

Após cada fatia, o projeto deve passar no `foundry-doctor`:
...
### Regra 3: Save Point Pattern (foundry-release-discipline)

Commit após cada tarefa que passa no foundry-doctor. Isso cria save points auditáveis:
...
```bash
git diff docs/foundry/manifest.json
...
Ao executar `/novais-digital:tasks`, cada task tem um gate de pronto. Trate cada gate como o "commit" entre fatias:

```markdown
# tasks-{artifact_id}.md
## Onda 1 — Scaffolding
- [x] T1.1: Criar src/prompts/{artifact_id}/v1.md — Gate: arquivo existe
- [x] T1.2: Criar src/llm/adapters/{artifact_id}.ts — Gate: foundry-doctor passes
...
- [ ] `foundry-doctor.sh` passa sem FAILs
...
|---|---|
| "Vou testar tudo no final" | Bugs se compõem. Um bug na Fatia 1 corrompe as Fatias 2–5. `foundry-doctor` a cada fatia. |
...
- [ ] `foundry-doctor.sh` passa completamente
