---
name: sdk-migration
description: Gerencia migração de SDKs externos, modelos depreciados e atualizações do próprio Foundry framework em projetos consumidores. Use quando o Anthropic SDK, LANGSMITH ou Prisma bumpar uma versão com breaking changes, quando um modelo LLM for depreciado pela Anthropic, ou quando um consumer project precisar sincronizar para uma nova versão do Foundry. Adaptado de deprecation-and-migration (agent-skills).
tier: 1
