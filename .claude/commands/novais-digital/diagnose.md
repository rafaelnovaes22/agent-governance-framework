...
# /novais-digital:diagnose — Fase 0 cobrável
...
project_type: <agentic_saas | platform | automation | hybrid>  # default: lê de docs/foundry/project.json; se ausente, agentic_saas
ai_enabled: <true | false>                                       # default: lê de project.json; se ausente, true
```

> **Resolução de project_type** (v0.2.0): se `--project_type` ausente, command tenta `docs/foundry/project.json`. Se também ausente, aplica defaults retroativos `agentic_saas` + `ai_enabled=true` e emite WARN sugerindo criar `project.json` a partir de `templates/project.template.json`.

## Execução

```
1. Trace start: trace_id = foundry.trace.start("/novais-digital:diagnose", {client_id, ...})

2. Helpers Tier 1:
   - Se __foundry_cache.dna vazio → invocar @company-dna
   - Se __foundry_cache.icp vazio → invocar @icp-loader
   - Se __foundry_cache.offerings vazio → invocar @offerings-loader

3. Resolver project_type:
   - Se --project_type fornecido → usar
   - Senão, ler docs/foundry/project.json → project.type / project.ai_enabled
...
4. Conduzir @diagnostic-runner com os 10 blocos do roteiro (interpretação por project_type):
   ...
   10. Próximos passos (GO/NO-GO + valor diagnóstico) + tipo de spec a gerar:
       - agentic_saas → /novais-digital:spec --type=platform-sku|product
       - platform → /novais-digital:spec --type=platform-module
       - automation → /novais-digital:spec --type=automation-job (futuro: alias de platform-module)

4. Persistir docs/clients/{client_id}/diagnostic.md
   (template: templates/diagnostic-spec.template.md)

5. Trace end: foundry.trace.end(trace_id, status, metrics)
...
```yaml
command: /novais-digital:diagnose
...
baseline_handoff:
  ...
  ready_for: "/novais-digital:unit-economics" | "/novais-digital:spec --type=platform-module"
...
next_step: "/novais-digital:spec --type=<tipo-resolvido> --client_id=<> --artifact_id=<>"
```
...
- [x] Frontmatter inclui `foundry_skill_version` e `foundry_command_version`
- [x] `trace_id` não-nulo (C6)
- [x] Nenhuma leitura registrada em paths Tier 3 (`runs/`, `outcomes/`, `eval-cases/`, `traces/`)

Se algum item falhar → `status: error` ou `status: partial` com `partial: true` no relatório; **não** propaga para `/novais-digital:spec` automaticamente.
...
`/novais-digital:diagnose --session_minutes=10` é compatível com sessões rápidas de qualificação preliminar. `--session_minutes=90` é o default para sessão completa com CEO. Em ambos os casos, **todos os 10 blocos** são executados.
