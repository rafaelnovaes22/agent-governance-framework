...
8. Emitir eventos WireLog (se project.telemetry.analytics_provider=wirelog):
   - foundry_promotion_requested: no início da transição com from_mode, to_mode, artifact_id, lifecycle_stage
   - foundry_gate_failed: para cada gate que falhar com gate_id, error_code, from_mode, to_mode
   - foundry_promotion_approved: se todos os gates passaram e transição foi registrada
   - foundry_promotion_blocked: se transição foi rejeitada (ilegal ou gates failed) — emitir ao invés de approved
