...
# /novais-digital:implement — Execução guiada
...
3. Verificar que agentes do módulo estão inicializados:
   ...
   → Se não: executar /novais-digital:aios-init --module {módulo} primeiro.

4. Executar pipeline via AIOS:
   /novais-digital:aios-run --module {módulo}
...
5. Aguardar aprovação humana em cada gate antes de prosseguir para próxima onda.
   Gates definidos em /novais-digital:aios-run — não pular.
```
...
**Não faz deploy, não inicia SHADOW, não promove modo.** Essas três ações ficam para `/novais-digital:promote` com aprovação humana.
...
4. Working tree limpo (sem mudanças não-commitadas) **OU** branch dedicada `foundry/implement-{artifact_id}`
...
2. Validar pre-conditions; se branch dedicada não existir, sugerir:
   git checkout -b foundry/implement-{artifact_id}
...
```ts
// AUTO-GERADO POR /novais-digital:implement
...
```ts
// AUTO-GERADO POR /novais-digital:implement — handler do artefato.
...
next_step: "/novais-digital:eval --artifact_id=<>" | "/novais-digital:promote --artifact_id=<>"
```
...
|---|---|---|
| "Vou iniciar SHADOW direto pra ganhar tempo" | Quebra ownership; SHADOW start é decisão de promotion-officer com aprovação humana | `/novais-digital:implement` para em Wave 4 prep; `start` é responsabilidade de `/novais-digital:promote` |
...
| "Edit no master direto, branch é overhead" | Implement gera 10+ arquivos; revert difícil sem branch | Pre-condition exige working tree limpo OU branch `foundry/implement-*` |
| "Skip recalc_unit_economics_required, é só warning" | C3 desatualizada vira drift silencioso | Output sempre `recalc_unit_economics_required: true`; flag visível no próximo `/novais-digital:promote` |
...
| 0.2.0 | 2026-05-06 | Foundry-6: suporte a `--via aios` + detecção automática de `spec.aios_tier` |
