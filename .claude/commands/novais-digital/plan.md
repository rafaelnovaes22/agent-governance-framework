---
description: Gera plano técnico do artefato/módulo. Para agentic (ai_enabled=true): define camadas, abstração de modelo (C7), instrumentação LANGSMITH (C6), TenantContext (C8), cronograma SHADOW. Para platform (ai_enabled=false): define camadas de serviço, audit log (C6), abstração de integração (C7), cronograma PILOT. Lê docs/foundry/project.json para detectar tipo automaticamente.
...
# /novais-digital:plan — Plano técnico
...
> Esta command **não escolhe modelo nem framework final**. `target_model` é advisory; decisão concreta vem em ADR-002 do projeto consumidor (Foundry-3) ou por escolha do dev. Quem aqui escolhe modelo cedo demais quebra C7.

## Pre-conditions

**Comuns:**
1. `docs/specs/{artifact_id}.md` existe com `c2_validation: pass`
2. `__foundry_cache.offerings` disponível

**Agentic (ai_enabled=true):**
3. `c4_thresholds` declarados na spec (output de `/novais-digital:sla-threshold`)
...
4. `audit_log_provider` declarado em `docs/foundry/project.json`
...
foundry_command_version: plan@0.1.0
linked_principles: [C5, C6, C7, C8]
...
> **Imports do SDK do provider proibidos fora de `src/llm/adapters/`**. Lint regex enforce no `pre-merge-check` (Foundry-2 F2.3).
...
- Proibido: `if (tenant_id === 'novais-digital')`, `switch (tenant_name)` (lint regex enforce)
...
- Próximo passo: `/novais-digital:tasks --artifact_id=<>`
```
...
spec_path: docs/specs/<>.md
foundry_command_version: plan@0.2.0
...
trace_id: <>
generated_at: <>
next_step: "/novais-digital:tasks --artifact_id=<>"
```
...
| "Instrumentação só em produção, dev fica simples" | Quebra C6; sem trace em dev, eval suite não simula realidade | Trace em todos ambientes; flag de amostragem aceitável só em prod AUTONOMOUS pós Foundry-4 |
...
| 0.2.0 | 2026-05-08 | Foundry-9: lê docs/foundry/project.json; ramo platform (seções 2P/4P/6P); output com plan_variant; pre-conditions bifurcadas (F9.14) |
