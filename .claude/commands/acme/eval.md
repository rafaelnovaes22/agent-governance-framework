...
Executa **toda a eval suite** (ou subset) contra o `prompts/{artifact_id}/v{version}/system.md` atual, compara cada output com o gabarito do case, e produz relatório auditável de **pass rate por outcome_category**. É a precondição mecânica para `/acme:promote` (gate "eval suite passing") e para `/acme:pre-merge-check` quando prompt mudou.

> Cada `eval run` é uma instância **datada e versionada**. Reviewer DeepAgent (Forge-3) audita histórico para detectar drift entre `prompt_hash` em produção e último eval run com aquele hash.
...
next_step: "/acme:promote --artifact_id=<> --to_mode=assisted" | "investigar fails antes de promover"
```
