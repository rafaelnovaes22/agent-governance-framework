# Auditoria de Pendências — Acme Forge

> **Data**: 2026-05-13
> **Versão auditada**: v0.12.0 (Forge-12 Fase 2)
> **Trigger**: primeira adoção real do framework por consumidor (Acme Social)
...
A v0.12.0 concluiu **toda a camada Surface** (HELLO, quickstarts, scripts/forge, PLAYGROUND, COMMON_ERRORS, friendly-errors hook) e o framework tecnicamente cobre os 4 `project_type` × 2 `ai_enabled`. **Mas a primeira adoção real (Acme Social, hoje) expôs que o framework ainda fala como criador, não como consumidor**: `forge-doctor` foi escrito assumindo que é executado dentro do próprio repo canônico, e o consumidor recebe ~65 falsos warnings + 2 fails que confundem o operador. Também foi expandido em ondas (Forge-9/10/11/12) com algumas inconsistências de metadados que se acumularam sem auditoria — manifest tem 7 entries com `version` desatualizado vs `manifest_version: 0.12.0`, e há **uma colisão real de IDs**: existem dois "F26" em `decisions.md` (linha 10 — Forge-9; linha 527 — Forge-10). Por fim, **dois itens de UX que o master-prompt promete (forge-router + persona auto-detect) ainda não existem**, o que mantém a porta de entrada `vibe` mais aspiracional do que operacional.

Resumo em números: **6 P0** (bloqueadores reais), **9 P1** (degradam UX), **8 P2** (nice-to-have) = **23 pendências catalogadas**, das quais 8 já estão no roadmap (Forge-12 Fase 3 + Forge-5 dependências) e 15 são novas/escondidas.

---

## Rota proposta

1. **Forge-13 — Consumer-mode hardening** (~3-4 dias): resolve P0/P1 que bloqueiam adoção (forge-doctor `--consumer`, sync script forge→consumidor, correção de F26 duplicado + metadados obsoletos, fixes de descrições no manifest). **Sem novas capabilities** — apenas torna o framework "consumível" sem fricção de mantenedor.
2. **Forge-14 — Surface Layer Fase 3** (~3-5 dias): entrega `forge-router` subagent + persona auto-detect + GLOSSARY_PLAIN standalone + PLAYGROUND/04-automation. Completa promessa de UX feita em Forge-11/12.
3. **Forge-15 — Real-world validation onda 1** (~5-7 dias): adoção formal por Aicfo e SchoolPlatform + primeira auditoria mensal real do `school-platform` (F9.13) + primeiro playbook vertical do Acme Social pós-AUTONOMOUS.
4. **Forge-16+ longer-term**: plugin marketplace (reaval F21), publicação npm/pip, multi-language reviewer (Anthropic + OpenAI + Gemini).

---

## Riscos

- **Sync forge↔consumidores ad-hoc**: cada bump de versão exige operador lembrar de `cp -r` artefatos canônicos. Sem script de sync + validação `framework_version_required`, drift silencioso é certo em ≤3 meses.
- **forge-router pode virar scope creep**: parse de linguagem natural → comando é fronteira ambígua. Sem critério claro de "quando o router se rende e devolve controle", risco de virar reescrita do master-prompt.
...
- **LGPD/GDPR no sync script**: se o futuro `forge sync` ler `.env` ou logs do consumidor para validação, vira coletor de dados sensíveis. Manter scope exclusivamente em `docs/forge/`, `.claude/`, `templates/`, `hooks/`, `scripts/` — **nunca tocar src/, .env, data/**.
- **C7 (portability)**: nenhuma das pendências propostas pode acoplar provider específico. Em especial, `forge-router` não pode assumir Claude Code (precisa funcionar com DeepAgent também).

---

## Próximo passo

**Decisão pendente do mantenedor (rafael@acme)**:
- Aprovar/ajustar agrupamento Forge-13/14/15
- Decidir tratamento de F26 duplicado (manter ambos + adicionar `(a)`/`(b)` ou renomear segundo como F26-bis)
- Priorizar entre Forge-13 (consumer hardening) e Forge-14 (Surface Fase 3) — opinião do auditor: **Forge-13 primeiro**, porque sem ele a adoção de Aicfo/SchoolPlatform vai sangrar.

---

## Outputs esperados

- ✅ Este documento (`docs/forge/AUDIT_2026-05-13_pendencias.md`) — entregue
- ⏳ Roadmap.md atualizado com Forge-13/14/15 — **aguarda decisão**
- ⏳ Issues GitHub abertas para cada P0 — **aguarda decisão**
- ⏳ Patch `forge-doctor --consumer` se Forge-13 aprovada — **fora do escopo deste audit**
...
### F30 — `forge-doctor.sh --consumer` mode
- **Nome**: Modo consumidor do forge-doctor (suprime falsos órfãos)
- **Por que importa**: Em Acme Social hoje, `forge-doctor` gera ~65 warnings de "artefato órfão" (porque manifest local não duplica entries do framework) + 2 FAILs sobre `reviewer/` ausente. Operador novo vê tela vermelha logo na primeira execução e desconfia do framework inteiro.
- **Esforço**: 4-6h (adicionar flag `--consumer`/auto-detect via ausência de `docs/forge/` próprio do framework, condicionar checks C6/reviewer)
...
- **Nome**: Renomear segunda F26 (Forge-10 TDD) para F26-bis ou similar
...
### F32 — Script `forge sync` para consumidores
- **Nome**: Sync artefatos canônicos forge → consumidor com validação de versão
- **Por que importa**: Hoje `cp -r` manual. Sem isso, todo bump (12 ondas em 7 semanas) vai gerar drift silencioso em consumidores. Bloqueia também rollback de versão e auditoria de "qual versão do Forge cada cliente está rodando".
...
- **Por que importa**: `forge-decisions.description` diz "F1-F26" mas chega em F29; `forge-roadmap.description` diz "7 ondas" mas são 12; `forge-decisions.version: 0.8.0`, `forge-roadmap.version: 0.6.0`, `forge-manifest.version: 0.9.0`, `claude-md-meta.version: 0.9.0`, `install.version: 0.9.0`, `changelog.version: 0.9.0` — manifest está em 0.12.0. Reviewer faz match por versão → relatórios podem reportar drift falso ou desatualizado.
- **Esforço**: 2-3h (varredura + atualização + recálculo sha256 + validação `forge-doctor.sh`)
- **Bloqueia**: precisão do reviewer mensal (F9.13 — primeira auditoria real)
- **Tipo**: bug (drift documental)

### F34 — `forge-router` subagent
- **Nome**: Subagent que traduz linguagem natural → pipeline `/acme:*`
- **Por que importa**: Master-prompt promete "operador CEO vibecoder não precisa conhecer slash commands" mas hoje precisa. Promessa quebrada de Forge-11/12. Quebra a porta de entrada `vibe`.
- **Esforço**: 1.5-2 dias (criar `.claude/agents/forge-router.md` Sonnet, decidir 6-8 intents canônicos, mapeamento intent→comando+args, fallback para "não entendi, sugiro X")
...
- **Por que importa**: Hoje exige `forge mode vibe`. CEO real nunca vai rodar esse comando. Sem auto-detect, modo vibe é teoricamente disponível mas nunca acionado.
- **Esforço**: 1 dia (heurística simples: linguagem do prompt + presença de jargão técnico + comandos digitados nos últimos N turnos; persistir em `.forge-mode` mas com `auto:true`; permitir override)
...
- **Por que importa**: Forge-9 declarou `project_type=automation` como suportado, mas PLAYGROUND tem só 3 (agentic/platform/hybrid). Consumidor RPA não tem walkthrough.
...
- **Nome**: Check em `forge-doctor.sh` (rodando no consumidor) para detectar `framework_version_required` < `framework.version` canônico
- **Por que importa**: Detectar consumidores desatualizados sem precisar do humano rodar sync manualmente. Complementar a F32.
- **Esforço**: 3-4h (precisa de fonte de verdade publicada — pode ser arquivo `latest.json` no repo Forge servido via raw.githubusercontent ou commit hash recente)
- **Bloqueia**: nada agudo
- **Tipo**: feature
...
- **Esforço**: 2-3 dias (mas precisa de Acme Social ou outro em AUTONOMOUS primeiro — não é trabalho do framework, é trabalho do consumidor)
- **Bloqueia**: validação real de Forge-5
...
- **Tipo**: doc (responsabilidade do consumidor — Acme Social ou SchoolPlatform)

### F40 — Primeira auditoria mensal real (`school-platform` platform)
- **Nome**: F9.13 — stress test da v0.8.x em projeto `platform`
- **Por que importa**: Forge-9 generalizou para platform mas nunca foi exercitada com auditoria real. Bugs prováveis no ramo `applies_when: ai_enabled=false`.
...
### F42 — `INSTALL.md` desatualizado vs realidade Forge-12
- **Nome**: Revisar INSTALL.md para refletir HELLO + scripts/forge
...
- **Por que importa**: 6 agentes com config JSON sem schema validado — drift entre eles é certo. forge-doctor C8 só checa `test_agent`.
...
- **Nome**: Expandir cobertura para 15+ padrões + observação real em Acme Social
- **Por que importa**: Forge-12 Fase 2 entregou 9 padrões mas critério de "validação real" não foi cumprido. Pode haver padrões críticos descobertos via uso (ex: erros do AIOS pipeline, erros do reviewer DeepAgent).
- **Esforço**: 0.5 dia (observar logs de Acme Social por 1-2 semanas + adicionar padrões emergentes)
...
- **Nome**: Reavaliação de publicar Forge como plugin Claude Code marketplace
...
### F50 — `forge upgrade` command
- **Nome**: Wrapper sobre `forge sync` que orquestra upgrade incluindo migração de schemas
...
**Comando faltando**: `/acme:sync` (alias para script F32) — para CEO/dev poder sincronizar Forge do consumidor sem sair do Claude Code. **Esforço estimado**: 1h se F32 já existir.

**Comando faltando candidato**: `/acme:status` — overview rápido (qual versão Forge, project_type, subscriptions ativas, gates pendentes). Hoje precisa rodar 3-4 comandos para ter essa visão. **Esforço**: 4-6h.
...
|---|---|---|
| prompt.template.md | 0.3.0 | ✅ Cobre F26 (Forge-9), mas **NÃO menciona Forge-10 TDD-first nem Forge-11/12** explicitamente |
| output-schema.json | 0.2.0 | ✅ |
| validation-rules.json | 0.3.0 | ✅ |
| 10 SKILL.md convertidas | varia | ✅ Forge-9.12 atualizou applies_when |

**Gap crítico**: prompt v0.3.0 não foi atualizado para Forge-10/11/12. Auditoria mensal real pode não checar:
- TDD red phase (Forge-10) — coverage por tier
- Master prompt instalado no consumidor (Forge-11)
- friendly-errors hook ativo (Forge-12 Fase 2)
...
| Check forge-doctor | Workflow CI cobre? |
|---|---|
...
**Gap**: Workflow `forge-validate` é template — consumidor precisa copiar. **Não há equivalente rodando no próprio repo agent-governance-framework**. Reviewer não verifica que o repo canônico passa nos seus próprios gates. **Recomendação**: copiar template para `.github/workflows/forge-validate.yml` no próprio repo Forge (dogfooding).

---

# Seção 3 — Pendências documentadas no roadmap

Itens marcados ⏳ em `docs/forge/roadmap.md`, classificados:

## ✅ Manter (ainda fazem sentido)

| ID | Item | Por que manter |
|---|---|---|
| F1.6 | 4 skills Acme-específicas em `examples/acme/skills/` | Não-bloqueador, mas útil quando Acme SaaS² for migrado |
...
| F11.6, F11.7 | Adoção Aicfo/SchoolPlatform + forge-router | Renomeados para F34 (router) e dependência de Forge-15 |
...
| ID | Item | Mudança |
|---|---|---|
| F11.6 | "Adoção pelos 3 consumidores" | Hoje 1/3 adotou (Acme Social). Escopo reduz para Aicfo + SchoolPlatform apenas. |
...
## Acme Social — ✅ Adotou hoje (2026-05-13)
...
- Criar `docs/forge/project.json` (Aicfo é provavelmente `hybrid` — plataforma com módulos IA)
- Adaptar `CLAUDE.md` local
- Rodar `forge-doctor` no consumidor (**vai falhar com problemas conhecidos** — exige F30 antes)

**Bloqueado por**: F30 (forge-doctor --consumer), F32 (sync) preferível mas não obrigatório
...
**Esforço**: 1-2 dias (assumindo SchoolPlatform ou Acme Social adotaram)
...
|---|---|---|
| **F26 duplicado** em `decisions.md` (linha 10 — Forge-9; linha 527 — Forge-10) | 🔴 Alto | F31 |
| **7+ entries com `version` obsoleto no manifest** (forge-decisions 0.8.0, forge-roadmap 0.6.0, forge-manifest 0.9.0, changelog 0.9.0, install 0.9.0, claude-md-meta 0.9.0) vs `manifest_version: 0.12.0` | 🟡 Médio | F33 |
| **Descrição `forge-decisions` desatualizada** ("F1-F26" mas chega em F29) | 🟡 Médio | F33 |
| **Descrição `forge-roadmap` desatualizada** ("7 ondas" mas são 12) | 🟡 Médio | F33 |
| **Reviewer prompt v0.3.0** não cita Forge-10/11/12 → auditoria não vai validar TDD/master-prompt/friendly-errors | 🔴 Alto | F33-bis (anexar a F33) |
| **AIOS templates só `test_agent` validado** em forge-doctor C8 — outros 5 templates podem driftar | 🟡 Médio | F43 |
| **forge-doctor.sh fala "C1-C8"** mas são checks internos do doctor, não C1-C8 da Constitution — nomenclatura colide e confunde leitor | 🟢 Baixo | Renomear como D1-D8 |
| **Repo Forge não roda seus próprios workflows** (templates/cicd/* nunca foi copiado para .github/workflows do agent-governance-framework) | 🟡 Médio | Dogfooding |
...
- Dogfooding: copiar `templates/cicd/*` para `.github/workflows/` do repo Forge — 0.25d
...
## Forge-14 — Surface Layer Fase 3 (~3-4 dias)

**Objetivo**: cumprir promessas de Forge-11/12 sobre porta de entrada `vibe`/`agent`.

**Entregáveis**:
- F34 — `forge-router` subagent (6-8 intents, fallback explícito) — 1.5d
...
- F44 — Hook friendly-errors expandido com padrões emergentes pós-Acme Social — 0.5d
- F41 — CI workflow valida walkthroughs (com LLM mock) — 1d

**Bump**: MINOR (v0.13.0 → v0.14.0)

**Dependências**: Forge-13 (sem ele forge-router cai em sync drift)

**Risco scope creep**: forge-router precisa ter contrato claro "quando me rendo". Limitar v1 a 6-8 intents.

---

## Forge-15 — Real-world validation onda 1 (~5-7 dias)

**Objetivo**: validar framework em produção com >1 consumidor + primeira auditoria real platform.

**Entregáveis**:
- F39 — ADR-002 assinada (responsabilidade SchoolPlatform ou Acme Social) — 1d
...
**Dependências**: Forge-13 + Forge-14
...
## Forge-16+ longer-term (escopo aberto)
...
- F50 — `forge upgrade` orquestrador
- F51 — i18n master-prompt (EN/ES)
- F52 — Forge dashboard HTML
...
**Caminho crítico recomendado**: Forge-13 (consumer hardening) → Forge-14 (surface Fase 3) → Forge-15 (real-world validation). Total estimado: **11-15 dias úteis**.

**Decisão imediata pendente** (mantenedor):
1. Aprovar agrupamento Forge-13/14/15 ou propor alternativo
2. Decidir tratamento de F26 duplicado (F26-bis vs renomear)
3. Priorizar Forge-13 antes de Forge-14 (recomendado pelo auditor)
