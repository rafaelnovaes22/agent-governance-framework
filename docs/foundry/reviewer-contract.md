...
> **Status**: ⏳ Especificação inicial (Foundry-0). Implementação técnica em Foundry-3. Atualizado em Foundry-9 (delivery-type agnostic) e Foundry-21 (analytics_provider WireLog).
> **Versão**: 0.3.0
> **Data**: 2026-05-26
...
### 3.2. Project config (NOVO em v0.2.0 — Foundry-9; atualizado em v0.3.0 — Foundry-21)
- **Arquivo**: `docs/foundry/project.json` (no consumidor)
...
- **Conteúdo crítico**:
  ...
  - `telemetry.analytics_provider` — eventos de negócio/outcomes (wirelog/posthog/segment/custom/null) **[NOVO Foundry-21]**
...
### 3.8. Documentação Foundry
- Tudo em `docs/foundry/`

### 3.9. Analytics provider — eventos de negócio (NOVO em v0.3.0 — Foundry-21)
...
- **O que o reviewer cruza**:
  - `outcomes_delivered_db` ↔ eventos `foundry_outcome_delivered` no WireLog: desvio ≤ 1% PASS / ≤ 5% WARN / > 5% FAIL
  - `outcomes_billed_db` ↔ eventos `foundry_outcome_billed`: mesma regra
  - Verificação de ausência de PII crua em sample de 20 eventos
  - Eventos `foundry_gate_failed` contêm `gate_id`, `artifact_id`, `lifecycle_stage`
