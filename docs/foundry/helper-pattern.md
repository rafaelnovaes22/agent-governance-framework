# Helper Pattern (BMAD) — Novais Digital Foundry

> **Versão**: 0.1.0
> **Data**: 2026-04-30
> **Origem**: `aj-geddes/claude-code-bmad-skills` (absorvido em Foundry-0)
...
**Meta documentada**: ≥ 70% de redução de tokens em prompts Tier 2/3 que dependem de Tier 1, medido via LangSmith após Foundry-3.
...
Cache por skill, namespace plano sob `__foundry_cache`:
...
Skill consumidora referencia `__foundry_cache.<key>` em vez de chamar a skill emissora se já estiver cacheado.

> Implementação concreta do cache vem em **Foundry-4** (hooks runtime). Até lá, o "cache" é convenção: skills L1/L2 declaram `requires: [@company-dna]` no frontmatter e o orquestrador (manual ou via slash command) garante uma única chamada.
...
Excecionalmente, em produção pós Foundry-4, pode haver cache cross-run com TTL curto (≤ 5 min) e invalidação por hash do arquivo. Não é o caso na fase Foundry-1.
...
No corpo da skill, leitura via `__foundry_cache`:

```markdown
## Inputs Tier 1 (via helper pattern)

- DNA: `__foundry_cache.dna` (provido por @company-dna)
- ICP: `__foundry_cache.icp` (provido por @icp-loader)
...
O reviewer DeepAgent (Foundry-3+) valida mensalmente:
...
Regra correspondente em `reviewer/validation-rules.json` será adicionada em Foundry-3 quando o reviewer for implementado.
...
| Versão | Data | Mudança |
|---|---|---|
| 0.1.0 | 2026-04-30 | Versão inicial — documenta padrão usado pelas 3 skills L0 da onda Foundry-1 (parcial) |
