...
### v0.6.0 (Foundry-21) — WireLog analytics_provider — F56
...
- **C6.analytics.1**: Desvio `outcomes_delivered_db` ↔ `foundry_outcome_delivered` no WireLog:
  - ≤ 1% → PASS
  - ≤ 5% → WARN
  - > 5% → FAIL
...
- **C6.analytics.3**: Eventos `foundry_gate_failed` incluem `gate_id`, `artifact_id`, `lifecycle_stage`. → WARN se ausentes.
...
```
funnel foundry_outcome_created -> foundry_outcome_delivered -> foundry_outcome_billed | last 30d
foundry_outcome_delivered | last 30d | count by day
foundry_gate_failed | last 30d | count by event_properties.gate_id
foundry_agent_error | last 30d | count by event_properties.error_code
foundry_promotion_blocked | last 90d | show eventType, toMode, errorCode, timestamp
```
