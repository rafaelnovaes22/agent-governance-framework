#!/usr/bin/env bash
# Acme Forge — forge-sync.sh
# Sincroniza artefatos canônicos do Forge para um projeto consumidor.
# Uso: bash scripts/forge-sync.sh [--from <path>] [--dry-run] [--force]
# Exit: 0 = OK, 1 = WARN (drift sem --force), 2 = FAIL
#
# Por padrão, --from é detectado automaticamente:
#   - Se este script roda DENTRO do repo Forge canônico (manifest.framework.canonical=true),
...
#     ../agent-governance-framework/ adjacente, depois $FORGE_PATH env, depois falha pedindo --from.
#
# O que sincroniza:
#   - .claude/CONSTITUTION.md
#   - .claude/{agents,commands,skills}/**
#   - hooks/**
#   - scripts/forge-doctor.sh, scripts/forge
#   - templates/**
...
#   - docs/forge/manifest.json (consumer mantém o seu — apenas atualiza framework_version_required)
#   - docs/forge/project.json
...
is_canonical() {
  local m_input="${1:-docs/forge/manifest.json}"
  [[ -f "$m_input" ]] || return 1
  local m
...
if is_canonical "$CONSUMER_ROOT/docs/forge/manifest.json"; then
  echo "❌ forge-sync.sh é para projetos consumidores, não para o repo canônico."
  echo "   Você está dentro do Forge canônico (manifest.framework.canonical=true)."
...
if [[ -z "$FROM" ]]; then
  ...
  # Tenta diretório adjacente comum
    ...
    echo "❌ Não consegui resolver fonte canônica do Forge."
    echo "   Passe --from <path/para/agent-governance-framework> ou defina FORGE_PATH."
    exit 2
  fi
fi
...
if ! is_canonical "$FROM/docs/forge/manifest.json"; then
  echo "❌ $FROM não é o repo canônico do Forge (manifest.framework.canonical != true)."
  exit 2
fi

# ─── Versão canônica e consumer ──────────────────────────────────────
FROM_NODE_MANIFEST="$(to_node_path "$FROM/docs/forge/manifest.json")"
CANONICAL_VERSION=$(node -e "console.log(JSON.parse(require('fs').readFileSync('$FROM_NODE_MANIFEST','utf8')).framework.version)")
CONSUMER_VERSION=""
CONSUMER_REQUIRED=""
if [[ -f "$CONSUMER_ROOT/docs/forge/manifest.json" ]]; then
  CONSUMER_NODE_MANIFEST="$(to_node_path "$CONSUMER_ROOT/docs/forge/manifest.json")"
...
if [[ -f "$CONSUMER_ROOT/docs/forge/manifest.json" ]] && [[ "$DRY_RUN" == "false" ]]; then
  echo ""
...
# ─── Audit trail ─────────────────────────────────────────────────────
if [[ "$DRY_RUN" == "false" ]]; then
  mkdir -p "$CONSUMER_ROOT/docs/forge"
  HIST="$CONSUMER_ROOT/docs/forge/sync-history.md"
  if [[ ! -f "$HIST" ]]; then
    cat > "$HIST" <<HEAD
# Forge Sync — Histórico

Toda execução de \`scripts/forge-sync.sh\` registra uma entrada aqui.
...
printf '\nPróximos passos:\n'
printf '  1. bash scripts/forge-doctor.sh --consumer    # validar consumer\n'
printf '  2. git -C "%s" diff                           # revisar mudanças\n' "$CONSUMER_ROOT"
printf '  3. git -C "%s" commit -am "chore(forge): sync v%s"\n\n' "$CONSUMER_ROOT" "$CANONICAL_VERSION"
