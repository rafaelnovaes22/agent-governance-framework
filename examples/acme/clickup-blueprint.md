---
target_workspace: "Acme Governança"
...
# ClickUp Blueprint — Acme Governança

> Estrutura proposta do ClickUp **interno Acme** aplicando Sincra L0/L1/L2.
> Cliente final **nunca** acessa este ClickUp — é cockpit interno do time Acme.
...
**Justificativa**: Princípio C8 (Anti-customização) recomenda manter ClickUp **interno**. Cliente externo prefere produto próprio (Acme Fin web app, dashboards) a ClickUp da Acme.

> Origem: ADR-003 do projeto `acme-governanca-ia`.
...
```
🏢 Workspace: Acme Governança

📂 Space 1 — ESTRATÉGICO (L0, read-mostly)
   ├─ 📜 Acme DNA & Manifesto             (1-3 docs, raramente muda)
...
📂 Space 4 — PRODUTOS (L1+L2, Acme Fin + Educacional + futuros)
...
📂 Space 5 — ENGENHARIA INTERNA (L2)
   ├─ 🛠️  Forge ondas (Forge-1, -2, ...)
   ├─ 🧪 Pesquisa & Spike
   ├─ 🏗️  Refactors / Tech Debt
   └─ ✅ Sprint atual do time Acme
...
📂 Space 6 — AUDITORIA & GOVERNANÇA (L2)
   ...
   ├─ 🕳️  Bypass Log                          registros ACME_FORGE_BYPASS=incident
...
(Script ainda não implementado — entrega em Forge-3 — `scripts/clickup-bootstrap.ts`).

### Comportamento esperado

- Idempotente: rodar 2x produz mesmo resultado
- Não-destrutivo: Spaces/Listas existentes mantidos; só cria o que falta
- Loga drift no final em `docs/forge/audits/clickup-drift-{YYYY-MM-DD}.md`

---

## 6. Hook de drift detection (Forge-4)
...
│           └── output: docs/forge/audits/clickup-drift-{YYYY-MM-DD}.md
```
...
## 8. Workspace ClickUp atual da Acme
...
> Antes do bootstrap rodar, time Acme deve **mapear** o que já existe no workspace e marcar para migração ou remoção.
...
Esta sessão (a que escreveu o Forge v0.2) deixa o blueprint **especificado**; outra sessão executa.
...
- [ ] Webhook handler do projeto `acme-governanca-ia` em rota `/webhooks/clickup`
- [ ] Bootstrap script implementado (Forge-3)
- [ ] Hook drift-detector instalado (Forge-4)
