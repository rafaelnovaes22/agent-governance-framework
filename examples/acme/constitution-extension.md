# Constitution Extension — Acme

> Princípios **C9, C10, C11** específicos do contexto Acme.
> Extensão da Constitution genérica do Forge ([`.claude/CONSTITUTION.md`](../../.claude/CONSTITUTION.md)).
...
- **Constitution base**: 0.2.0 (Forge)
- **Extensão Acme**: 0.1.0
- **Data**: 2026-04-30

Mudanças em qualquer princípio C9-C11 exigem:
1. ADR no `acme-governanca-ia`
...
**Regra**: Acme mantém **duas estruturas de custo separadas**:
...
**Não misturar** ARPU médio "Acme total" — esconde realidade de cada track.
...
- Plataforma SKUs em `src/skus/` (no `acme-governanca-ia`)
- Produtos em diretórios próprios ou repos separados (`financeiro.acme.com.br` em Lovable+Supabase)
...
**Regra**: Toda oferta Acme pertence a **uma das 3 categorias** formalmente definidas:

1. **Acme Diagnóstico** (Categoria 1) — porta de entrada paga
2. **Acme Plataforma** (Categoria 2) — high-touch SaaS² com SKUs verticais
3. **Acme Produtos** (Categoria 3) — self-serve SaaS² padronizado
...
- Produtos: Acme Fin (Beta), Acme Educacional (Discovery)
...
| Produtos beta | Aceita stack divergente (ex: Acme Fin = Lovable+Supabase) **se** plano de migração existir |
...
**Cenário**: time Acme quer construir agente que ajude usuários do Acme Fin a entender melhor a Análise gerada.

Como Constitution + extensão se aplicam:

1. **C11**: agente pertence à categoria "Produtos" (Acme Fin é Produto) → aplicar regras de Produtos
2. **C1**: cliente Acme Fin **já passou** pela primeira análise (= diagnóstico de fato, conforme C1 com relax para Produtos) → OK
