...
- [ ] 🔴 **G6** — TDD-RED gate: todo módulo modificado em `src/{modules,features,domains}/` tem `tests/{module}/unit/` correspondente (Forge v0.9.0+)

## 3. Testes funcionais do projeto cliente (obrigatório para SHADOW e acima — Forge v0.9.0+)

> Esta seção é nova na Forge v0.9.0. O pipeline AIOS agora é TDD-first: testes são gerados
> antes do código. Esta seção garante que a CI roda esses testes de verdade e enforce coverage.

- [ ] 🔴 Workflow `forge-test.yml` ativo (copiado de `templates/cicd/github-actions-test.template.yml`)
...
- [ ] 🔴 Status checks obrigatórios: `forge-doctor`, `skill-security-scan`, `pre-merge-check`, `tdd-red-phase-check`, `unit-tests`, `integration-tests`
...
eval_last_run: null
audit_last_run: null
last_test_run:             # Forge v0.9.0+ — testes funcionais do projeto cliente
