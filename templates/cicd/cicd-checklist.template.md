...
- [ ] 🟡 Smoke test WireLog: job de auditoria verifica query `foundry_reviewer_audit_completed` no WireLog após execução (se `analytics_provider=wirelog`)
...
> **WireLog**: recomendado quando `analytics_provider=wirelog`. Não é obrigatório para CI/CD funcionar — adapter é no-op quando key ausente. Configurar para habilitar smoke test de analytics em CI (verificação de que `foundry_eval_completed` é emitido após job de eval).
