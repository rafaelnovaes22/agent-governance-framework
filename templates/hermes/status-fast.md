...
description: >
  ...
  foundry-headless.yml. Use APENAS para leitura de estado; nunca para execução.
...
Para qualquer coisa que precise **executar** um pipeline (audit, eval, pre-merge), use `foundry.skill.md`.
...
for CONSUMER in edu-platform aicfo clickup-automation marketing-ai-agents; do
  ...
  gh api "repos/novais-digital/$CONSUMER/contents/docs/foundry/project.json" \
    ...
    2>/dev/null || echo '{"status": "sem project.json (não Foundry ou não sincronizado)"}'
done
```
...
### 5. Últimos runs do foundry-headless (histórico de execuções)
...
```
📊 Status Novais Digital Foundry — [DATA]
...
[Se drift: "⚠️ X consumer(s) desatualizados — rode foundry-sync"]
```
