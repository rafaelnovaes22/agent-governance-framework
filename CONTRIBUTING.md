# Contributing — Novais Digital Foundry

> Como evoluir o framework. Lê quem **mantém** o Foundry ou contribui com mudanças (adiciona templates, skills, refinamentos na Constitution, etc.).
...
2. **Leia** [`docs/foundry/decisions.md`](./docs/foundry/decisions.md) para entender F1-F12
3. **Leia** [`docs/foundry/out-of-scope.md`](./docs/foundry/out-of-scope.md) para evitar adicionar algo deliberadamente excluído
4. **Conheça** o caso real em [`examples/novais-digital/`](./examples/novais-digital/)
...
2. Branch `foundry/template-{nome}`
...
4. Atualizar `docs/foundry/manifest.json` — adicionar entrada em `artifacts.templates[]`
...
2. Branch `foundry/skill-{nome}`
3. Criar arquivo em `.claude/skills/L{0|1|2}/{nome}.md` (Foundry-1)
...
2. Branch `foundry/command-{verbo}`
3. Criar `.claude/commands/{escopo}/{verbo}.md` (Foundry-2)
...
2. Branch `foundry/agent-{nome}`
3. Criar `.claude/agents/{nome}.md` (Foundry-3)
...
2. Branch `foundry/constitution-X.Y.Z`
3. ADR no projeto consumidor (não no Foundry) justificando
...
**Quando**: novo caso de uso real (não-Novais Digital) adota o Foundry e quer contribuir como exemplo.

**Processo**:
1. Pasta `examples/{nome-do-projeto}/` com conteúdo análogo a `examples/novais-digital/`
2. README explicando o contexto
3. Adaptar Constitution caso necessário (nunca alterar a do Foundry — adicionar `constitution-extension.md` próprio)
...
2. Branch `foundry/major-vX.0.0` com:
   - ADR formal em `docs/foundry/decisions.md` (próxima Fxx disponível)
   ...
   - Migration guide em `docs/foundry/migrations/vX.0.0-to-vY.0.0.md` com exemplos antes/depois
...
Quando v(N).0.0 sai, o framework mantém v(N-1).x.y em "security-only support" por **6 meses** ou até o último consumidor conhecido migrar, o que vier primeiro. Bugs sérios em v(N-1) recebem patch, mas features novas não. Documentar em `docs/foundry/support-policy.md` (se aplicável).

**Quando o último MAJOR aconteceu**: até v0.13.0 (Foundry-13 Sprint 1), **ZERO MAJOR bumps** ocorreram desde Foundry-0. Toda evolução foi feita via MINOR com matriz de interpretação ou flags backwards-compat. Isso é o caso desejado — preserve.
...
- Links relativos para outros arquivos do Foundry
...
- [ ] Branch nomeada conforme tipo (`foundry/template-X`, `foundry/skill-Y`, etc.)
- [ ] Manifest atualizado (`docs/foundry/manifest.json`)
...
- (Após Foundry-3) Reviewer DeepAgent pode ser invocado em PRs grandes para sanity check independente
...
- ❌ Adicionar dependência de stack específico (Node, Python, etc.) ao **core** do Foundry — Foundry é markdown + JSON + (Foundry-3+) scripts opcionais
- ❌ Inserir lógica Novais-específica em arquivos genéricos (`templates/`, `.claude/CONSTITUTION.md`, etc.) — vai para `examples/novais-digital/`
- ❌ Adicionar princípio C9+ à Constitution genérica — extensões viram `examples/{dominio}/constitution-extension.md`
- ❌ Modificar `examples/novais-digital/` sem confirmar com mantenedor da Novais Digital
...
## Roadmap das Ondas Foundry (referência)

Ver [`docs/foundry/roadmap.md`](./docs/foundry/roadmap.md) para detalhe.

| Onda | Foco | Status |
|---|---|---|
| Foundry-0 | Fundação | ✅ Concluída em v0.2.0 |
| Foundry-1 | Skills L0/L1/L2 | 🔜 Próxima |
| Foundry-2 | Slash commands | 🔜 Pendente |
| Foundry-3 | Subagents Guardian + reviewer | 🔜 Pendente |
| Foundry-4 | Hooks runtime | 🔜 Pendente |
| Foundry-5 | Playbooks verticais | 🔜 Pendente |
...
Repositório privado. Contribuições aceitas mediante autorização do mantenedor (Novais Digital / Novais Digital).
